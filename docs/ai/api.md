...
- `chat` response data: `{message, workflow[], show_recommendations, recommendation_reason, recommended_packages[], order_gate?}` (lihat `ChatResult`). Session identifier tidak dikembalikan di JSON; browser memakai HttpOnly cookie `vero_chat_session`.
...
- `done` — `ChatResult` utuh (`{message, workflow, show_recommendations, recommendation_reason, recommended_packages, order_gate?}`): terminal, klien finalisasi state (packages, flags, gate order).
