...
- **Koneksi zombie guard (BUG-4, 28 Jul 2026):** `WriteTimeout` server tetap `0` (demi SSE long-lived, lihat ARCH-3), sehingga tidak ada deadline tulis global. Tiga lapis pertahanan menggantikannya: (1) write-error detection per-tulis via `http.NewResponseController(c.Writer)` + `SetWriteDeadline(now+10s)` + `Flush()` — error pada koneksi setengah-putus (NAT timeout/laptop sleep) membuat handler keluar + subscriber dilepas; (2) max lifetime koneksi `sseMaxLifetime=30 menit` via `time.NewTimer`, lalu handler mengirim event `reconnect` dan return — `EventSource` browser reconnect otomatis; (3) cap subscriber `MaxSubscribers=100` di atas.
