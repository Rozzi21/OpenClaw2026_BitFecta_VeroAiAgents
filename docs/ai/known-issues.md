...
- **Account linking**: 1) match `sub` via ExternalIdentity → login; 2) match email terverifikasi (`email_verified=true` wajib) → link (buat ExternalIdentity + mirror `google_sub`); 3) tidak ada → buat `RoleUser` baru + ExternalIdentity atomik (`CreateUserWithGoogleIdentity`) dengan password bcrypt acak (pola SEC-24). Role tidak pernah dari luar (SEC-1).
